...
def request_valid_image(update: Update) -> None:
    ...
    update.message.reply_text("Please upload a valid photo (JPG format) for your receipt.")
...
def handle_non_image_file(update: Update, context: CallbackContext) -> None:
    ...
    if is_valid_non_image_file(file_type):
        update.message.reply_text("It looks like you uploaded a non-image file. Please upload a valid photo (JPG/PNG format).")
    else:
        update.message.reply_text("Unsupported file type. Please upload a JPG image for your receipt.")
...
def is_valid_non_image_file(file_type: str) -> bool:
    ...
    valid_file_types = ["application/pdf", "application/zip", "text/csv", "application/x-ipynb+json"]  
